...
training_set = df.iloc[:int(cps[0]-16), 1:2].values
test_set = df.iloc[1:int(cps[0]-16), 1:2].values
...
dataset_train = df.iloc[:int(cps[0]-16), 1:2]
dataset_test = df.iloc[int(cps[0]/2):int(cps[0]-16), 1:2]
dataset_total = pd.concat((dataset_train, dataset_test), axis = 0)
inputs = dataset_total[len(dataset_total) - len(dataset_test)-18:].values
